...
public class InitPrivilegeData implements ApplicationRunner {
  ...
  public void run(ApplicationArguments args) throws Exception {
    ...
    privilegeRepo.save(
        Privilege.builder().id("READ").description("읽기 권한").build());

    privilegeRepo.save(
        Privilege.builder().id("WRITE").description("쓰기 권한").build());

    privilegeRepo.save(
        Privilege.builder().id("DELETE").description("삭제 권한").build());

    privilegeRepo.save(
        Privilege.builder().id("UPDATE").description("수정 권한").build());

    privilegeRepo.save(
        Privilege.builder().id("DOWNLOAD").description("다운로드 권한").build());

    privilegeRepo.save(
        Privilege.builder().id("PRIVATE_DOWNLOAD").description("비공개 파일 접근 권한").build());
  }
}
